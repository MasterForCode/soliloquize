 ...
 * @date 2020/7/13
 */
public class Excels<T extends Workbook> {
    /**
     * HSSFWorkbook针对EXCEL 2003版本，扩展名为.xls，此种的局限就是导出的行数最多为65535行。因为导出行数受限，不足7万行，所以一般不会发送内存溢出(OOM)的情况
     * XSSFWorkbook这种形式的出现是由于第一种HSSF的局限性产生的，因为其导出行数较少，XSSFWorkbook应运而生，其对应的是EXCEL2007+ ，扩展名为.xlsx ，最多可以导出104万行，不过这样就伴随着一个问题–OOM内存溢出。因为使用XSSFWorkbook创建的book sheet row cell 等是存在内存中的，并没有持久化到磁盘上，那么随着数据量的增大，内存的需求量也就增大。那么很有可能出现 OOM了
...
//
//    public Excels(T workbook, String filePath) {
...
//        Excels<SXSSFWorkbook> t = new Excels<>();
//    }
...
//     * @return 生成的文件
//     */
//    public java.io.File createExcel(String fileName) {
//        fileName += Const.XLSX;
